...
def product_deletion(id, name):
    delete_query = f"DELETE FROM sells WHERE product_id = '{id}'"
...
def dashboard_redirect(records):
    ...
    product_list_query = f"SELECT p.*, s.quantity_sold FROM product p JOIN sells s ON p.product_id = s.product_id WHERE s.seller_id = '{st.session_state['auth']}';"
    ...
    for i in product_list:
        ...
        col2.write(i[4])
        col2.write(i[7])
        col2.button("delete item", args=(i[0],i[1]), on_click=product_deletion, key=cart_key)
        st.write("---")
